---
name: evolution-omics-paper-zh
description: Translate and critically read papers on new genes, de novo origin, evolutionary genomics, regulatory innovation and omics in Chinese. Produce a full translation and a separate Word report with phylogenetic, homology and functional-evidence appraisal.
metadata:
  version: "0.5.0"
---
...
4. 通读 [领域加强规范](references/domain.md)，按论文相关性选择检查项；领域结论仍须来源支持。
...
## 新增必读分支
精读报告默认按[后续科学问题](references/research-opportunities.md)提供3个优先问题与另外10个发散问题，和本文核心质疑分开。涉及组学时按[组学路由](references/omics-review.md)选择实际测量分支；本版先识别测量类型，确无组学测量时跳过该模块。仅在开发维护skill且获用户授权时，按[审核规范](references/independent-review.md)让DeepSeek独立审核skill；不送审论文，不在论文报告中附模型审核记录。这三项不能替代全文翻译、原图与中文图注、相关文献推荐和核心问题检索解答。
